...
ttW_fit = ttW_info['fit']
ttZ_fit = ttZ_info['fit']
...
for page in xrange(n):
    ...
    if args.checkboxes:
        ...
        pads = plot.MultiRatioSplitColumns([0.4,0.2,0.2], [0.,0.,0.,0.], [0.,0.,0.,0.])
        pads[2].SetGrid(1, 0)
        pads[3].SetGrid(1, 0)
    pads[0].SetGrid(1, 0)
    pads[0].SetTickx(1)
    ...
    g_impacts_hi_1 = ROOT.TGraphAsymmErrors(n_params)
    g_impacts_lo_1 = ROOT.TGraphAsymmErrors(n_params)
    g_impacts_hi_2 = ROOT.TGraphAsymmErrors(n_params)
    g_impacts_lo_2 = ROOT.TGraphAsymmErrors(n_params)
    g_impacts_hi_3 = ROOT.TGraphAsymmErrors(n_params)
    g_impacts_lo_3 = ROOT.TGraphAsymmErrors(n_params)
    ...
    for p in xrange(n_params):
        ...
        if pdata[p]['type'] != 'Unconstrained':
            ...
            redo_boxes.append(i)
        g_impacts_hi_1.SetPoint(i, 0, float(i) + 0.5)
        g_impacts_lo_1.SetPoint(i, 0, float(i) + 0.5)
        g_impacts_hi_2.SetPoint(i, 0, float(i) + 0.5)
        g_impacts_lo_2.SetPoint(i, 0, float(i) + 0.5)
        g_impacts_hi_3.SetPoint(i, 0, float(i) + 0.5)
        g_impacts_lo_3.SetPoint(i, 0, float(i) + 0.5)
        if args.checkboxes:
        ...
        imp_2 = pdata[p]["r_ttW"]
        imp_3 = pdata[p]["r_ttZ"]
        g_impacts_hi_1.SetPointError(i, 0, imp[2] - imp[1], 0.5, 0.5)
        g_impacts_lo_1.SetPointError(i, imp[1] - imp[0], 0, 0.5, 0.5)
        g_impacts_hi_2.SetPointError(i, 0, imp_2[2] - imp_2[1], 0.5, 0.5)
        g_impacts_lo_2.SetPointError(i, imp_2[1] - imp_2[0], 0, 0.5, 0.5)
        g_impacts_hi_3.SetPointError(i, 0, imp_3[2] - imp_3[1], 0.5, 0.5)
        g_impacts_lo_3.SetPointError(i, imp_3[1] - imp_3[0], 0, 0.5, 0.5)
        max_impact = max(
            max_impact, abs(imp[1] - imp[0]), abs(imp[2] - imp[1]))
        max_impact_2 = max(
            max_impact_2, abs(imp_2[1] - imp_2[0]), abs(imp_2[2] - imp_2[1]))
        max_impact_3 = max(
            max_impact, abs(imp_3[1] - imp_3[0]), abs(imp_3[2] - imp_3[1]))
        col = colors.get(tp, 2)
    ...
    # Go to the other pad and draw the impacts histo
    pads[2].cd()
    if max_impact_2 == 0.: max_impact_2 = 1E-6  # otherwise the plotting gets screwed up
    h_impacts_2 = ROOT.TH2F(
        "impacts_2", "impacts_2", 6, -max_impact_2 * 1.1, max_impact_2 * 1.1, n_params, 0, n_params)
    plot.Set(h_impacts_2.GetXaxis(), LabelSize=0.03, TitleSize=0.04, Ndivisions=505, Title=
             '#Delta#hat{r_ttW}'  )
    plot.Set(h_impacts_2.GetYaxis(), LabelSize=0, TickLength=0.0)
    h_impacts_2.Draw()
    ...
    h_impacts_3 = ROOT.TH2F(
        "impacts_3", "impacts_3", 6, -max_impact_3 * 1.1, max_impact_3 * 1.1, n_params, 0, n_params)
    plot.Set(h_impacts_3.GetXaxis(), LabelSize=0.03, TitleSize=0.04, Ndivisions=505, Title=
             '#Delta#hat{r_ttZ}' )
    plot.Set(h_impacts_3.GetYaxis(), LabelSize=0, TickLength=0.0)
    h_impacts_3.Draw()
    ...
    if 'method' in data and data['method'] in lo_color:
        method = data['method']
    g_impacts_hi_1.SetFillColor(plot.CreateTransparentColor(hi_color[method], alpha))
    g_impacts_hi_1.Draw('2SAME')
    g_impacts_lo_1.SetFillColor(plot.CreateTransparentColor(lo_color[method], alpha))
    g_impacts_lo_1.Draw('2SAME')
    pads[1].RedrawAxis()
    pads[2].cd()
    g_impacts_hi_2.SetFillColor(plot.CreateTransparentColor(hi_color[method], alpha))
    g_impacts_hi_2.Draw('2SAME')
    g_impacts_lo_2.SetFillColor(plot.CreateTransparentColor(lo_color[method], alpha))
    g_impacts_lo_2.Draw('2SAME')
    pads[2].RedrawAxis()
    pads[3].cd()
    g_impacts_hi_3.SetFillColor(plot.CreateTransparentColor(hi_color[method], alpha))
    g_impacts_hi_3.Draw('2SAME')
    g_impacts_lo_3.SetFillColor(plot.CreateTransparentColor(lo_color[method], alpha))
    g_impacts_lo_3.Draw('2SAME')
    pads[3].RedrawAxis()
    ...
    legend.AddEntry(g_impacts_hi_1, '+1#sigma Impact', 'F')
    legend.AddEntry(g_impacts_lo_1, '-1#sigma Impact', 'F')
    legend.Draw()
    ...
    s_nom1, s_hi1, s_lo1 = GetRounded(ttW_fit[1], ttW_fit[2] - ttW_fit[1], ttW_fit[1] - ttW_fit[0])
    s_nom2, s_hi2, s_lo2 = GetRounded(ttZ_fit[1], ttZ_fit[2] - ttZ_fit[1], ttZ_fit[1] - ttZ_fit[0])
    if not args.blind:
        ...
        plot.DrawTitle(pads[2], '#hat{ttW} = %s^{#plus%s}_{#minus%s}%s' % (
            s_nom1, s_hi1, s_lo1,
            '' if args.units is None else ' '+args.units), 3, 0.27)
        plot.DrawTitle(pads[3], '#hat{ttZ} = %s^{#plus%s}_{#minus%s}%s' % (
            s_nom2, s_hi2, s_lo2,
            '' if args.units is None else ' '+args.units), 3, 0.27)
