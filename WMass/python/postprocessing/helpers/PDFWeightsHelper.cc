...
void PDFWeightsHelper::Init(unsigned int nreplicas, unsigned int neigenvectors, const char* incsv) {  
  
  transformation_.resize(nreplicas,neigenvectors);
  
  std::ifstream instream(incsv);
  if (!instream.is_open()) {
    throw cms::Exception("PDFWeightsHelper")
      << "Could not open csv file" << incsv; 
  }
...
void PDFWeightsHelper::DoMC2Hessian(double nomweight, const double *inweights, double *outweights) {
