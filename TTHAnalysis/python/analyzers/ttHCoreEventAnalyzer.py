...
class ttHCoreEventAnalyzer( Analyzer ):
    ...
    def makeMT(self, event):
        MTObject = event.inclusiveLeptons + event.selectedTaus + getattr(event, 'selectedIsoTrack', [])
        for ltt in MTObject: # ltt stands for "lepton or tau or track"
            ltt.cosLMet = cos(ltt.phi() - event.met.phi())
            ltt.mt      = mtw(ltt, event.met)
            ltt.Q80     = 1 - 80**2/(2*ltt.pt()*event.met.pt())
        return