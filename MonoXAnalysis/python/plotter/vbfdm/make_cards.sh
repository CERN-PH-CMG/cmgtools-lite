...
OPTIONS=" --s2v -j $J -l ${LUMI} -f -X trigger -X metfilters "
test -d $OUTNAME || mkdir -p $OUTNAME
OPTIONS="${OPTIONS} --od cards/$OUTNAME ";

SYST="vbfdm/systsEnv.txt"

VARIABLE="'abs(JetClean1_eta-JetClean2_eta)' '[0.5,1,2,2.5,3,3.5,4,4.5,5,6,7,10]'"
...
if [[ "$1" == "all" || "$1" == "ZM" ]] ; then
    ...
    command="python makeShapeCards.py ${DOFILE} vbfdm/mca-80X-muonCR.txt vbfdm/zmumu.txt ${VARIABLE} $SYST $OPTIONS_ZM --od ${OUTNAME} --correlateProcessCR 'ZNuNu,SR,rfac_ZLL_full,templates/rfactors_ZNuNuSR_Over_zmumuCR.root' --region ZM --unbinned --xp ZLL,EWKZLL "
    echo "===> EXECUTING " $command
    eval $command    
    echo "Done at $(date)"
fi;
...
if [[ "$1" == "all" || "$1" == "ZE" ]] ; then
    ...
    command="python makeShapeCards.py ${DOFILE} vbfdm/mca-80X-electronCR.txt vbfdm/zee.txt ${VARIABLE} $SYST $OPTIONS_ZE --od ${OUTNAME} --correlateProcessCR 'ZNuNu,SR,rfac_ZLL_full,templates/rfactors_ZNuNuSR_Over_zeeCR.root' --region ZE --unbinned --xp ZLL,EWKZLL "
    echo "===> EXECUTING " $command
    eval $command    
    echo "Done at $(date)"
fi;
...
if [[ "$1" == "all" || "$1" == "WM" ]] ; then
    ...
    command="python makeShapeCards.py ${DOFILE} vbfdm/mca-80X-muonCR.txt vbfdm/wmunu.txt ${VARIABLE} $SYST $OPTIONS_WM --od ${OUTNAME} --correlateProcessCR 'W,SR,rfac_W_full,templates/rfactors_WSR_Over_wmunuCR.root' --region WM --unbinned --xp W,EWKW "
    echo "===> EXECUTING " $command
    eval $command
    echo "Done at $(date)"
fi;
...
if [[ "$1" == "all" || "$1" == "WE" ]] ; then
    ...
    command="python makeShapeCards.py ${DOFILE} vbfdm/mca-80X-electronCR.txt vbfdm/wenu.txt ${VARIABLE} $SYST $OPTIONS_WE --od ${OUTNAME} --correlateProcessCR 'W,SR,rfac_W_full,templates/rfactors_WSR_Over_wenuCR.root' --region WE --unbinned --xp W,EWKW "
    echo "===> EXECUTING " $command
    eval $command
    echo "Done at $(date)"
fi;
