...
CDEN="(tag_IsoMu20 || tag_IsoMu20_eta2p1 ) && tag_pt > 20 && outerValidHits "
for match in dr03e03; do
    for tk in tk0 tk; do
        for PostFix in '' _NoZ; do
            ...
            python tnpEfficiency.py $PDS -d "pt > 15 && $CDEN"                 -n "$NUM" $OPTS --x-var abseta $TWOBINS      -N mu_${match}_${tk}${PostFix}_pt15_two      $MASS --xtitle "|#eta|" ;
            python tnpEfficiency.py $PDS -d "pt > 15 && $CDEN"                 -n "$NUM" $OPTS --x-var eta $EBINS           -N mu_${match}_${tk}${PostFix}_pt15_eta      $MASS --xtitle "#eta" ;
            python tnpEfficiency.py $PDS -d "pt > 15 && $CDEN && abseta < 2.4" -n "$NUM" $OPTS --x-var tag_nVertices $VBINS -N mu_${match}_${tk}${PostFix}_pt15eta24_vtx $MASS --xtitle "N(vertices)";
        done;
    done;
done
