...
#dowhat = "dumps" 
#dowhat = "yields" 
#dowhat = "ntuple" # syntax: python top_eft/ttH_plots.py no 2lss_SR_extr outfile_{cname}.root --sP var1,var2,...
dojeccomps=True
...
TREESALL = "--xf GGHZZ4L_new,qqHZZ4L,tWll,WW_DPS,WpWpJJ,WWW_ll,T_sch_lep,GluGluToHHTo2V2Tau,TGJets_lep,WWTo2L2Nu_DPS,GluGluToHHTo4Tau,ZGTo2LG,GluGluToHHTo4V,TTTW --FMCs {P}/0_jmeUnc_v1  --FMCs {P}/2_btagSF --FMCs {P}/2_scalefactors_lep/  --Fs {P}/3_tauCount/ --Fs {P}/8_FOvars/ " 
YEARDIR=YEAR if YEAR != 'all' else ''
TREESONLYFULL     = "-P "+P0+"/NanoTrees_UL_v2_060422/%s          --Fs  {P}/1_recl "%(YEARDIR,)         
TREESONLYSKIM     = "-P "+P0+"/NanoTrees_UL_v2_060422_skim2lss/%s   --Fs {P}/1_recl_allvars_new "%(YEARDIR,)


def base(selection):
    THETREES = TREESALL
    CORE=' '.join([THETREES,TREESONLYSKIM])
    CORE+=" -f -j %d -l %s  -L top_eft/funcionsEFT.cc --tree NanoAOD --mcc top_eft/lepchoice-ttH-FO.txt --split-factor=-1 --WA prescaleFromSkim --year %s  --mcc top_eft/mcc-METFixEE2017.txt"%(nCores, lumis[YEAR],YEAR if YEAR!='all' else '2016APV,2016,2017,2018')# --neg" --s2v 
    ...
    if selection=='2lss':
        GO="%s top_eft/mca-2lss-mc.txt top_eft/2lss_tight.txt "%CORE
        GO="%s -W 'L1PreFiringWeight_Nom*puWeight*btagSF*leptonSF_2lss*triggerSF_2lss'"%GO
        if dowhat in ["plots","ntuple"]: GO+=" top_eft/2lss_3l_plots.txt --xP '^lep(3|4)_.*' --xP '^(3|4)lep_.*' --xP 'kinMVA_3l_.*' "
        ...
        GO="%s top_eft/mca-3l-mc.txt top_eft/3l_tight.txt "%CORE
        GO="%s -W 'L1PreFiringWeight_Nom*puWeight*btagSF*leptonSF_3l*triggerSF_3l'"%GO
        if dowhat in ["plots","ntuple"]: GO+=" top_eft/2lss_3l_plots.txt --xP '^(2|4)lep_.*' --xP '^lep4_.*' --xP 'kinMVA_2lss_.*' "
        if dowhat == "plots": GO=GO.replace(LEGEND, " --legendColumns 3 --legendWidth 0.42 ")
        GO += " --binname 3l "
    elif selection=='4l':
        GO="%s top_eft/mca-4l-mc.txt top_eft/4l_tight.txt "%CORE
        GO="%s -W 'L1PreFiringWeight_Nom*puWeight*btagSF*leptonSF_4l*triggerSF_4l'"%GO
        if dowhat in ["plots","ntuple"]: GO+=" top_eft/2lss_3l_plots.txt --xP '^(2|3)lep_.*' --xP '^lep(1|2|3|4)_.*' --xP 'kinMVA_.*' "
...
if __name__ == '__main__':
    ...
    if '2lss_' in torun:
        ...
        if '_legacy' in torun: x = x.replace('top_eft/2lss_tight.txt',"top_eft/2lss_tight_legacy.txt")
        if '_1fo' in torun:
        ...
        if '_unc' in torun:
            x = add(x,"--unc top_eft/systsUnc.txt  --xu CMS_ttHl_TTZ_lnU,CMS_ttHl_TTW_lnU")
    ...
    if '3l_' in torun and not('cr') in torun:
        ...
        if '_legacy' in torun: x = x.replace('top_eft/3l_tight.txt',"top_eft/3l_tight_legacy.txt")
        if '_1fo' in torun:
    ...
    if 'cr_trigger_eff' in torun:
        ...
        x = x.replace("-W 'L1PreFiringWeight_Nom*puWeight*btagSF*leptonSF_2lss*triggerSF_ttH(LepGood1_pdgId, LepGood1_conePt, LepGood2_pdgId, LepGood2_conePt, 2, year)'", '')
