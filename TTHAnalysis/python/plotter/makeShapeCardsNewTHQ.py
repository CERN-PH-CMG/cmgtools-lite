...
for binname, report in allreports.iteritems():
  ...
  datacard.write((npatt % 'process')+(" "*6)+(" ".join([kpatt % p[0:p.find("_1_m1")] if(p.find("_1_m1") >= 0) else p for p in procs]))+"\n") 
  ...
  for h in towrite:
      hname = h.GetName() 
      if (hname.find("_1_m1") >= 0): hname = hname[0:hname.find("_1_m1")] 
      h.SetName(hname)
