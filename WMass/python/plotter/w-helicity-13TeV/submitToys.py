...
if __name__ == "__main__":
    ...
    cmdBase = "combineTool.py -d {ws} -M MultiDimFit -t {nt} --expectSignal=1 -m 999 {savefr} --cminInitialHesse 1 --cminFinalHesse 1 --cminPreFit 1 --redefineSignalPOIs norm_W%s_long --floatOtherPOIs=0 --freezeNuisanceGroups efficiencies,fixedY --toysNoSystematics -n _{pfx} -s {seed} --trackParameters {track} --job-mode lxbatch --task-name {taskname} --sub-opts='-q 8nh' %s" % (charge, '--dry-run' if options.dryRun else '')

    if options.nTj==None or ntoys<options.nTj:
        cmd = cmdBase.format(nt=ntoys,ws=workspace,pfx=prefix,seed=12345,taskname='toys_'+prefix,track=trackPars,savefr='--saveFitResult')
        print cmd
        os.system(cmd)
    else:
        random.seed()
        ...
        for j in xrange(int(ntoys/nTj)):
            cmd = cmdBase.format(nt=nTj,ws=workspace,pfx=prefix+"_%d"%j,seed=int(random.uniform(0,1000*len(jobs))),
                                 taskname="toys_%s_%d"%(prefix,j),track=trackPars,savefr='--saveFitResult' if j==0 else '')
            os.system(cmd)
        cmd = cmdBase.format(nt=resT,ws=workspace,pfx=prefix+"_%d"%len(jobs),seed=int(random.uniform(0,1000*len(jobs))),
                             taskname="toys_%s_%d"%(prefix,len(jobs)),track=trackPars,savefr='')
        os.system(cmd)
