...
def collectProcesses(region, make):
...
parser.add_option("--lspam", dest="lspam", type="string", default="#bf{CMS} #it{Preliminary}", help="Left-Spam for mcPlots")
parser.add_option("--noRatio", dest="ratio", action="store_false", default=True, help="Do NOT plot the ratio (i.e. give flag --showRatio)")

base = "python mcPlots.py {MCA} {CUTS} {PLOTFILE} -P {T} --neg --s2v --tree {TREENAME} -f -j 4 --lspam '{LSPAM}' --legendWidth 0.20 --legendFontSize 0.035 {MCCS} {MACROS} {RATIO} -l {LUMI} --pdir {O} {FRIENDS} {PROCS} {PLOTS} {FLAGS} --showMCError"
...
for p in plots:
	for m in makes:
		...
		procs   = collectProcesses(mm.region, m)
