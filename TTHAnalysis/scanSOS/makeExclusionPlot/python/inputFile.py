...
class inputFile():
    ...
    def findATTRIBUTE(self, fileName, attribute):
        ...
        for line in fileIN:
            ...
            fileIN.close()
            return ('' if len(tmpLINE) < 2 else tmpLINE[1])
    ...
    def findEXPECTED(self, fileName):
        ...
        for line in fileIN:
            ...
            fileIN.close()
            rootFileIn = rt.TFile.Open(tmpLINE[1])
            print 'tis the rootfile', tmpLINE[1]
            print 'nominal will be', rootFileIn.Get(tmpLINE[2])
            return {'nominal': rootFileIn.Get(tmpLINE[2]),
                    'plus': rootFileIn.Get(tmpLINE[3]),
                    'minus': rootFileIn.Get(tmpLINE[4]),
                    'colorLine': tmpLINE[5],
                    'colorArea': tmpLINE[6],# changed ,
                    'plus2': rootFileIn.Get(tmpLINE[7]),
                    'minus2': rootFileIn.Get(tmpLINE[8])}#all me
