...
if [[ "$RM" == 0 ]]; then
    ...
    sed 's|cmgResubChunk -q 8nh |cmgResubChunk -q 2nd |g' clean2.sh > clean2bckp.sh
else
    sed 's|cmgResubChunk -q 8nh |rm -r |g' clean2.sh > clean2bckp.sh
fi;

if [ $saveList = "true" ]; then 
    echo "#Saving file to resubmit/remove bad chunks"
    cp clean2bckp.sh chunksToResub.sh 
fi
