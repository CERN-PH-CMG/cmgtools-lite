...
cuts['common'] = '(HLT_MU||HLT_ELE)&&Flag_goodVertices&&Flag_CSCTightHaloFilter&&Flag_HBHENoiseFilter&&Flag_HBHENoiseIsoFilter&&Flag_eeBadScFilter&&lnujj_nOtherLeptons==0&&(abs(lnujj_l1_l_pdgId)==11||abs(lnujj_l1_l_pdgId)==13&&lnujj_l1_l_relIso04<0.1)'

cuts['common'] = '(((HLT2_MU||HLT2_ELE||HLT2_ISOMU||HLT2_ISOELE||HLT2_MET120)&&run>2000)||(run<2000))&&Flag_goodVertices&&Flag_CSCTightHaloFilter&&Flag_HBHENoiseFilter&&Flag_HBHENoiseIsoFilter&&Flag_eeBadScFilter&&lnujj_nOtherLeptons==0&&Flag_badChargedHadronFilter&&Flag_badMuonFilter&&(abs(lnujj_l1_l_pdgId)==11||(abs(lnujj_l1_l_pdgId)==13&&lnujj_l1_l_relIso04<0.1))'
...
def makeSignalShapesMVV(filename,template):
    for l in leptons:
        ...
        print 'Making JSON'
        cmd='vvMakeJSON.py  -o "{jsonFile}" -g "MEAN:pol1,SIGMA:pol2,ALPHA1:pol3,N1:pol0,ALPHA2:pol4,N2:pol0" -m 600 -M 5000  {rootFile}  '.format(jsonFile=jsonFile,rootFile=rootFile)
        os.system(cmd)
...
def makeSignalShapesMJJ(filename,template):
    for p in purities:
        ...
        cmd='vvMakeSignalMJJShapes.py -s "{template}" -c "{cut}"  -o "{rootFile}" -V "lnujj_l2_pruned_mass"  samples'.format(template=template,cut=cut,rootFile=rootFile,minMJJ=minMJJ,maxMJJ=maxMJJ)
        os.system(cmd)
        jsonFile=filename+"_MJJ_"+p+".json"
        cmd='vvMakeJSON.py  -o "{jsonFile}" -g "mean:pol4,sigma:pol4,alpha:pol3,n:pol0,slope:pol4,f:pol4" -m 600 -M 5000  {rootFile}  '.format(jsonFile=jsonFile,rootFile=rootFile)
        os.system(cmd)
...
def makeBackgroundShapesMVVNoP(name,filename,template,addCut=""):
    for l in leptons:
        ...
        cmd='vvMakeBackgroundMVVConditionalShapes.py -s "{template}" -c "{cut}"  -o "{rootFile}" -v "lnujj_LV_mass" -V "lnujj_l2_pruned_mass"  -b 200  -x {minMVV} -X {maxMVV}   -B 12 -y {minMJJ} -Y {maxMJJ}   samples'.format(template=template,cut=cut,rootFile=mvvFile,minMVV=minMVV,maxMVV=maxMVV,minMJJ=minMJJ-12,maxMJJ=maxMJJ+12)
        os.system(cmd)
...
def makeBackgroundShapesMVVPerCat(name,filename,template,addCut=""):
    for l in leptons:
        for c in categories:
            ...
            mvvFile=filename+"_MVV_"+name+"_"+l+"_"+c
            cmd='vvMakeBackgroundMVVConditionalShapes.py -s "{template}" -c "{cut}"  -o "{rootFile}" -v "lnujj_LV_mass" -V "lnujj_l2_pruned_mass"  -b 500  -x {minMVV} -X {maxMVV}   -B 10 -y {minMJJ} -Y {maxMJJ}   samples'.format(template=template,cut=cut,rootFile=mvvFile,minMVV=minMVV,maxMVV=maxMVV,minMJJ=minMJJ-15,maxMJJ=maxMJJ+15)
#                cmd='vvMakeBackgroundMVVConditionalShapesErfPowSim.py -s "{template}" -c "{cut}"  -o "{rootFile}" -v "lnujj_LV_mass" -V "lnujj_l2_pruned_mass"  -b 500  -x {minMVV} -X {maxMVV}   -B 120 -y {minMJJ} -Y {maxMJJ}   samples'.format(template=template,cut=cut,rootFile=mvvFile,minMVV=minMVV,maxMVV=maxMVV,minMJJ=minMJJ,maxMJJ=maxMJJ)
            os.system(cmd)


            rootFile=filename+"_MVVHist_"+name+"_"+l+"_"+c+".root"
            cmd="vvPDFToHisto.py -n histo -s 'slopeSyst_{name}:1.0:0.1' -m 'meanSyst0_{name}:1.0:0.1,meanSyst1_{name}:mjj:5e-4' -w 'widthSyst_{name}:1.0:0.1' -o '{rootFile}' -b {binsMVV} -x {minMVV} -X {maxMVV} -B {binsMJJ} -y {minMJJ} -Y {maxMJJ} {mvvFile}.json".format(name=name+"_"+l+"_"+c,rootFile=rootFile,binsMVV=binsMVV,minMVV=minMVV,maxMVV=maxMVV,binsMJJ=binsMJJ,minMJJ=minMJJ,maxMJJ=maxMJJ,mvvFile=mvvFile) 
            os.system(cmd)
...
def makeTopShapes2(name,filename,template,addCut=""):
    ...
    for p in purities:
        ...
        cmd='vvMakeTopMJJConditionalShapesFromTruth.py -s "{template}" -c "{cut}"  -o "{rootFile}" -v "lnujj_l2_pruned_mass" -V "lnujj_LV_mass"  -b 20  -x {minMJJ} -X {maxMJJ}  -i "LNuJJ_XWW_MJJ_{purity}.json"   samples'.format(template=template,cut=cut,rootFile=mjjFile,minMJJ=minMJJ,maxMJJ=maxMJJ,purity=p)
#        os.system(cmd)
...
def makeTopShapes(name,filename,template,addCut=""):
    ...
    for p in purities:
        ...
        mjjFile=filename+"_MJJ_"+name+"_"+p
        cmd='vvMakeTopMJJConditionalShapes.py -s "{template}" -c "{cut}"  -o "{rootFile}" -v "lnujj_l2_pruned_mass" -V "lnujj_LV_mass"  -b 20  -x {minMJJ} -X {maxMJJ}  samples'.format(template=template,cut=cut,rootFile=mjjFile,minMJJ=minMJJ,maxMJJ=maxMJJ)
        os.system(cmd)
...
#makeTopShapes2("topRes","LNuJJ",topTemplateExt,"lnujj_l2_mergedVTruth==1&&lnujj_l2_nearestBDRTruth>0.8")
#makeBackgroundShapesMJJNoL("topNonRes","LNuJJ",topTemplateExt,"!(lnujj_l2_mergedVTruth&&lnujj_l2_nearestBDRTruth>0.8)")
#makeBackgroundShapesMVVPerCat("topNonRes","LNuJJ",topTemplateExt,"!(lnujj_l2_mergedVTruth&&lnujj_l2_nearestBDRTruth>0.8)")
