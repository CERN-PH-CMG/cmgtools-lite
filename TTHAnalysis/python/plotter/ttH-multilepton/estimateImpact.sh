...
#combineTool.py -M Impacts -d $HERE/${DATACARD} --doInitialFit --robustFit 1 $ASIMOV $EXTRA -m 1 -n $PREFIX --redefineSignalPOIs r_ttH --floatOtherPOIs 1 


echo "Second Stage: fit scan for each nuisance parameter"
echo "--------------------------------------------------"

#combineTool.py -M Impacts -d $HERE/${DATACARD} --robustFit 1 --doFits $ASIMOV $EXTRA -m 1 -n $PREFIX  --redefineSignalPOIs r_ttH --job-mode SGE

echo "Third Stage: collect outputs"
echo "----------------------------"

combineTool.py -M Impacts -d $HERE/${DATACARD} -o impacts$PREFIX.json $ASIMOV $EXTRA -m 1 -n $PREFIX --redefineSignalPOIs r_ttH
