...
keep_vars = ['n_jets', 'weight', 'met_pt', 'mt', 'pthiggs', 'delta_phi_l1_l2', 'delta_eta_l1_l2', 'vbf_deta', 'vbf_mjj', 'vbf_dphidijethiggs', 'vbf_jdphi', 'vbf_n_central', 'svfit_transverse_mass', 'l1_pt', 'l2_pt']
...
for cut in cuts:
    for sample in all_samples:
        ...
        for i in xrange(tree_out.GetEntries()):
            tree_out.GetEntry(i)
            full_weight[0] = formula.EvalInstance()
            # print full_weight[0]
            # new_b.Fill()
            weight_tree.Fill()
            # tree_out.Fill()
