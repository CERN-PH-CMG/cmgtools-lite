...
def tau21DDT(x):
    mass = x.softDropMassCor
    tau1 = x.userFloat('ak8PFJetsPuppiValueMap:NjettinessAK8PuppiTau1')
    tau2 = x.userFloat('ak8PFJetsPuppiValueMap:NjettinessAK8PuppiTau2')
    pt = x.pt()
    if mass>0 and pt>0 and tau1!=0:
...
FatJetType = NTupleObjectType("FatJetType", baseObjectTypes=[jetType], variables = [
    NTupleVariable("tau1",   lambda x : x.userFloat('ak8PFJetsPuppiValueMap:NjettinessAK8PuppiTau1'), float),
    NTupleVariable("tau2",   lambda x : x.userFloat('ak8PFJetsPuppiValueMap:NjettinessAK8PuppiTau2'), float),
    NTupleVariable("tau3",   lambda x : x.userFloat('ak8PFJetsPuppiValueMap:NjettinessAK8PuppiTau3'), float),
#    NTupleVariable("tau4",   lambda x : x.userFloat('ak8PFJetsPuppiValueMap:NjettinessAK8PuppiTau4'), float),
...
#    NTupleVariable("btagBOOSTED",   lambda x : x.btag("pfBoostedDoubleSecondaryVertexAK8BJetTags"), float),
    NTupleVariable("btagBOOSTED", lambda x : x.Hbbtag if hasattr(x,'Hbbtag') else -1.0, float),
    ...
    NTupleVariable("maxQuarkDistance",   lambda x : x.maxQuarkDistance, float,"",-99.0,True),
    ...
    NTupleSubObject("gen_quark1", lambda x : x.genqq[0].p4() if len(x.genqq)>0 else None, fourVectorType,True, nillable=True),
    NTupleSubObject("gen_quark2", lambda x : x.genqq[1].p4() if len(x.genqq)>1 else None, fourVectorType,True, nillable=True),
    NTupleSubObject("gen_qq", lambda x : (x.genqq[0].p4()+x.genqq[1].p4()) if len(x.genqq)>1 else None, fourVectorType,True, nillable=True),
    NTupleSubObject("gen_b", lambda x : x.genb[0].p4() if len(x.genb)>0 else None, fourVectorType,True, nillable=True)
])
