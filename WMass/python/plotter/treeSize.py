...
var data = [ {scriptdata} ];
""".format(allsize=sizeline, filename=os.path.basename(sys.argv[1]), scriptdata=trees["tree"].scriptdata)
print """
...
grandtotal = trees["tree"].allsize; runningtotal = 0
for s in trees["tree"].survey:
...
<h1>tree detail</h1>
"""
for s in sorted(trees["tree"].survey, key = lambda s : s.name):
