...
ttH) 
    ...
    PBASE="plots/80X/${ANALYSIS}_Moriond17/lepMVA/v2.1-dev/fr-mc/"
    ;;
susy) 
...
for WP in $WPs; do
	...
	VETOCONVERSIONS="LepGood_mcPromptGamma==0"
        ...
        case $WP in # 075ib1f30E2ptc30
            ...
            0??iv32*) SelDen="-A pt20 den '$SIP8 && $VCSVM && (LepGood_mvaTTH > $WNUM || (abs(LepGood_pdgId)==13 && $VCSVVL && LepGood_segmentCompatibility > 0.3) || (abs(LepGood_pdgId)==11 && $VCSVVL))'";  Num="mvaPt_${WP%%i*}"i; XVar="mvaPt${WP%%i*}";;
	...
	case $WP in
	    ...
	    *X5*) SelDen="$SelDen -A pt20 vcsvle '(LepGood_mvaSUSY > ${WNUM} && LepGood_mediumMuonId>0) || (${VCSVL} && ${ELEMVAPRESEL} && $PTF30)'"; Num="${Num%%X*}"; XVar="${XVar%%X*}";;
        ...
        case $WP in
            ...
            *Eptc30) SelDen="$SelDen -A pt20 eidden '(abs(LepGood_pdgId) == 13 || LepGood_idEmu || LepGood_pt*if3(LepGood_mvaTTH>0.75, 1.0, 0.80/LepGood_jetPtRatiov2) < 30)'"; XVar="${XVar%%E*}";;
            *E2) SelDen="$SelDen -A pt20 eidden LepGood_idEmu2"; Num="${Num%%E*}"; XVar="${XVar%%E*}";;
            *E2ptc30) SelDen="$SelDen -A pt20 eidden '(abs(LepGood_pdgId) == 13 || LepGood_idEmu2 || LepGood_pt*if3(LepGood_mvaTTH>0.75, 1.0, 0.80/LepGood_jetPtRatiov2) < 30)'"; XVar="${XVar%%E*}";;
	...
	B1="${B1} --mcc ttH-multilepton/mcc-eleIdEmu2.txt  "
        ...
        MuFakeVsPt="$MuDen ${BDen} --sP '${ptJI}_${XVar}_coarse' --sp TT_red --xcut 10 999" 
        ElFakeVsPt="$ElDen ${BDen} --sP '${ptJI}_${XVar}_coarse' --sp TT_red --xcut 20 999" 
...
#	break;
        ...
        MuFakeVsPtLongBin="$MuDen ${BDen} --sP '${ptJI}_${XVar}_coarselongbin' --sp TT_red --xcut 10 999" 
        ElFakeVsPtLongBin="$ElDen ${BDen} --sP '${ptJI}_${XVar}_coarselongbin' --sp TT_red --xcut 20 999" 
	...
	#break;
        ...
        echo "( $B0 $ElFakeVsPt -p TT_red,TT_bjets,QCDEl_red_El12,QCDEl_bjets -o $PBASE/$what/el_bnbe_${Me}_eta_00_15.root  -R pt20 eta 'abs(LepGood_eta)<1.479' ${BG} )"
        echo "( $B0 $ElFakeVsPt -p TT_red,TT_bjets,QCDEl_red_El12,QCDEl_bjets -o $PBASE/$what/el_bnbe_${Me}_eta_15_25.root  -R pt20 eta 'abs(LepGood_eta)>1.479' ${BG} )"
       ...
       MuFakeVsPt0J="$MuDen --sP '${ptJI}_${XVar}_coarse' --sp TT_red --xcut 10 999" 
       ElFakeVsPt0J="$ElDen --sP '${ptJI}_${XVar}_coarse' --sp TT_red --xcut 20 999" 
       ...
       ##AwayJet b-tag
       MuFakeVsPtB="$MuDen --sP '${ptJI}_${XVar}_coarse,pt_coarse' --sp TT_red,TT_SSbt_black ${BDen} --xcut 10 999" 
       ElFakeVsPtB="$ElDen --sP '${ptJI}_${XVar}_coarse,pt_coarse' --sp TT_red,TT_SSbt_black ${BDen} --xcut 20 999" 
       ...
       echo "( $B0 $ElFakeVsPtB -p 'QCDEl_red,QCDEl_red_ajb.*' -o $PBASE/$what/el_qajb_${Me}_eta_15_25.root -R pt20 eta 'abs(LepGood_eta)>1.479' ${BG} )"
        ...
        ElFakeVsPt="$ElDen ${BDen} --sP '${ptJI}_${XVar}_coarse' --sp TT_red --xcut 10 999" 
        ...
        echo "( $B0 $ElFakeVsPt -p QCDEl_red_pt[0-9]+,QCDEl_red_El[0-9]+ -o $PBASE/$what/el_hltid_${Me}_eta_00_15.root -R pt20 eta 'abs(LepGood_eta)<1.479' ${BG} )"
        echo "( $B0 $ElFakeVsPt -p QCDEl_red_pt[0-9]+,QCDEl_red_El[0-9]+ -o $PBASE/$what/el_hltid_${Me}_eta_15_25.root -R pt20 eta 'abs(LepGood_eta)>1.479' ${BG} )"
        for C in 45_999; do
            conePtCut="-A pt20 conePt '${C%_*} < LepGood_pt*if3(LepGood_mvaTTH>${WNUM},1,0.80/LepGood_jetPtRatiov2) && LepGood_pt*if3(LepGood_mvaTTH>${WNUM},1,0.80/LepGood_jetPtRatiov2) < ${C#*_}' ";
