...
T="/afs/cern.ch/user/f/folguera/workdir/trees/ewkino/TREES_76X_160502/"
J=12;
...
OPTIONS="${OPTIONS} --mcc susy-ewkino/lepchoice-ss2l-FO.txt --mcc susy-ewkino/susy_ss2l_triggerdefs.txt " # neg necessary for subsequent rebin
CATPOSTFIX=""
...
if [[ "$1" == "ss2l" ]]; then
    OPT_2L="${OPTIONS} -E met100"
    
    echo "making cards for ss2l"
    python makeShapeCardsSusy.py susy-ewkino/mca-ss2l-mc_forlimits${SPLITDECAYS}.txt susy-ewkino/susy_ss2l_cuts.txt $FUNCTION_2L $SYSTS $OPT_2L -o ss2l ; 
    echo "Done at $(date)"
fi
