...
DATA=$(for I in $(seq 1 7); do echo $P/TnPTree_v41_76X_RunD_part${I}.root; done)
MC=$(for I in $(seq 1 2); do echo " --refmc $P/TnPTree_76X_DYLL_M50_MadGraphMLM_part${I}.root"; done)
PDS="$DATA $MC"
...
for ID in LooseIdOnly; do # SIP4 SIP8 LooseFromIso LooseIdOnly; do
  if [[ "$SEL" != "" ]] && echo $SEL | grep -q -v $ID; then continue; fi
  case $ID in
      SIP4) NUM="SIP < 4 && abs(dzPV) < 1.0 && abs(dB) < 0.50" ; CDEN="$CDEN && Loose " ;;
      SIP8) NUM="SIP < 8 && abs(dzPV) < 0.1 && abs(dB) < 0.05" ; CDEN="$CDEN && Loose " ;;
      LooseIdOnly)  NUM="Loose" ; CDEN="$CDEN && (Glb || TM) ";  XBINS="[5,10,15,20]" ;;
      LooseFromIso) NUM="Loose" ; CDEN="$CDEN && isoTrk03Rel < 0.2 " ;  XBINS="[5,10,15,20]" ;;
  esac;
  for BMOD in bern4 expo ; do # bern3  #
    if [[ "$SEL" != "" ]] && echo $SEL | grep -q "_" && echo $SEL | grep -q -v $BMOD; then continue; fi
    for SMOD in BWDCB dvoigt ; do #   MCTG
    if [[ "$SEL" != "" ]] && echo $SEL | grep -q "_" && echo $SEL | grep -q -v $BMOD; then continue; fi
    if [[ "$SMOD" == "MCTG" ]]; then OPTS="${OPTS} --fine-binning 100  "; fi
        ...
        if [[ "$SMOD" == "dvoigt" && "$BMOD" == "bern7" ]]; then
        ...
        if [[ "$SMOD" == "dvoigt" && "$BMOD" == "bern4" ]]; then
