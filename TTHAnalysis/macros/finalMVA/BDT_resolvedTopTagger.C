...
class BDT_rTT_Jet : public BDT_rTT_ptvec{
 public:
  ...
  BDT_rTT_Jet(float pt,float eta, float phi, float mass, float deepcsv, float cvsl, float cvsb, float ptD, float axis1, int mult):
    BDT_rTT_ptvec(pt,eta,phi,mass), _deepcsv(deepcsv), _cvsl(cvsl), _cvsb(cvsb), _ptD(ptD), _axis1(std::exp(-axis1)), _mult(mult){};
...
public:
  ...
  BDT_rTT_top(std::shared_ptr<BDT_rTT_Jet> _b, std::shared_ptr<BDT_rTT_Jet> _j2, std::shared_ptr<BDT_rTT_Jet> _j3){
    ...
    *p4 = (b!=nullptr) ? *p4w+*(dynamic_cast<BDT_rTT_ptvec*>(b.get())) : *p4w;
  }
...
void BDT_resolvedTopTagger::Init(std::string weight_file_name){
  ...
  reco::details::loadTMVAWeights(TMVAReader_.get(),"BDT",weight_file_name);
...
std::vector<float> BDT_resolvedTopTagger::EvalMVA(){
  ...
  for (int i1=0; i1<njets; i1++) {
	...
	if (i1==i2) continue;
	if (jets.at(i1)->deepcsv()<jets.at(i2)->deepcsv()) continue;
          for (int i3=0; i3<njets; i3++){
	    if (i1==i3 || i2==i3) continue;
	    if (jets.at(i2)->pt()<jets.at(i3)->pt()) continue;
	    if (jets.at(i1)->deepcsv()<jets.at(i3)->deepcsv()) continue;
  ...
  auto time = stopWatch.RealTime();
  if (time>0.1) std::cout << Form("Slow event: done %d jets, %d good permutations, in %.3f seconds",njets,int(allcands.size()),time) << std::endl;
  return output;
